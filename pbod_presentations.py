 ...
 ("prebattle_custom_deployment", 0, mesh_load_window, [
    (ti_on_presentation_load,
     [(set_fixed_point_multiplier, 1000),
		...
		(assign, reg60, ":num_player_troops_in_battle"),
	  ...
	  (create_text_overlay, reg0, "@You will have {reg60} troops available at the battle's start", tf_center_justify|tf_single_line),
      (position_set_x, pos1, 500),
	  ...
	  (store_mul, ":cur_y", ":num_of_stacks", ":cur_y_adder"),
		...
		(assign, reg59, ":in_count"),
		(create_text_overlay, "$g_presentation_obj_custom_battle_designer_17", "@{reg59}", tf_with_outline),
		(try_begin), 		
		    (gt, ":in_count", reg60),
			(overlay_set_color, "$g_presentation_obj_custom_battle_designer_17", 0xFF1100),
		(else_try),
		    (overlay_set_color, "$g_presentation_obj_custom_battle_designer_17", 0xFFFFFF),
		(try_end),
		...
		(create_text_overlay, reg0, "@of {reg60} troops", 0),
     ...
     [
       ...
       (store_trigger_param_2, ":value"),
		...
		(else_try),
			...
			(assign, reg59, 0),
			(overlay_set_text, "$g_presentation_obj_custom_battle_designer_17", "@0"),
			(overlay_set_color, "$g_presentation_obj_custom_battle_designer_17", 0xFFFFFF),
		(else_try),
		    (eq, ":object", "$g_presentation_obj_custom_battle_designer_19"),
			(assign, ":cur_count", reg59),
			(assign, ":round_size", reg60),
		...
		(else_try), #Number Boxes
		 ...
		 (overlay_set_text, "$g_presentation_obj_custom_battle_designer_17", "@{reg0}"),
		 (try_begin),
			(gt, ":new_total", ":round_size"),
			(overlay_set_color, "$g_presentation_obj_custom_battle_designer_17", 0xFF1100),
		 (else_try),
 ...
 ("pbod_preferences", 0, mesh_load_window, [
    (ti_on_presentation_load,
      [
		...
		(create_text_overlay, reg0, "@Enable Real Deployment Phase:", tf_vertical_align_center),
        (position_set_y, pos1, ":headings_y"),
        (overlay_set_position, reg0, pos1),
		(val_sub, ":headings_y", ":cur_y_shift"),
		...
		(assign, "$g_presentation_obj_name_kingdom_1", reg0), #to lazily account for the removal of the battle size overlay
		# (val_sub, ":inputs_y", 9),	
		...
		(create_check_box_overlay, reg0, "mesh_checkbox_off", "mesh_checkbox_on"), #Real Deployment
        (position_set_y, pos1, ":inputs_y"),
        (overlay_set_position, reg0, pos1),	
        (val_sub, ":inputs_y", ":cur_y_shift"),	
		...
		(party_get_slot, ":pref_deploy", "p_main_party", slot_party_pref_real_deployment),
		...
		(val_add, ":overlay", 1),
		(overlay_set_val, ":overlay", ":pref_deploy"),
		(val_add, ":overlay", 1),
      ...
      [
		...
		(else_try),
		  (val_add, ":overlay", 1),
		  (eq, ":object", ":overlay"), #Deployment
		  (party_set_slot, "p_main_party", slot_party_pref_real_deployment, ":value"),
		(else_try),
		  (val_add, ":overlay", 1),
	  ...
	  [
		...
		(else_try),
			...
			(else_try),
			  ...
			  (this_or_next|eq, ":object", ":overlay_2"),
			  (eq, ":object", ":overlay"), #Deployment
			  (overlay_set_text, "$g_presentation_obj_banner_selection_1", "@Real Deployment allows you to position your troops on the field before the battle begins."),
			(else_try),
			  (val_add, ":overlay", 1),
			  (val_add, ":overlay_2", 1),
			  (this_or_next|eq, ":object", ":overlay_2"),
 ...
 ("pbod_redefine_keys", 0, mesh_load_window, 
  [(ti_on_presentation_load, 
    [(presentation_set_duration, 999999),
     ...
     (create_text_overlay, reg0, "@Keys Configuration", tf_center_justify), (overlay_set_color, reg0, 0x000000),
     (overlay_set_position, reg0, pos1), (overlay_set_size, reg0, pos2),
     # (position_set_y, pos1, 649),  (position_set_x, pos1, 499),  # Tittle Position
     # (create_text_overlay, reg0, "@Keys Configuration", tf_center_justify), (overlay_set_color, reg0, 0xFF0000),
     # (overlay_set_position, reg0, pos1), (overlay_set_size, reg0, pos2),	 
     ...
     (overlay_set_color, "$g_presentation_obj_custom_battle_designer_2", 0x678000),  # Note's Color  was 600000
     ...
     (else_try),
        (position_set_x, pos2, 750), (position_set_y, pos2, 750),   
         (position_set_x, pos1, 25),
     (try_end),     
     (position_set_x, pos3, 120),  (position_set_y, pos3, 28),   # KeyConfig's Buttons Size     was 30
	 ...
	 (create_game_button_overlay, "$g_presentation_obj_custom_battle_designer_6", "@Disable All"),                    # DISABLE ALL Button
	 # Rearrange Buttons
     (position_set_y, pos1, 25), 
	 (position_set_x, pos1, 900),         (overlay_set_position, "$g_presentation_obj_custom_battle_designer_3", pos1),
     (position_set_x, pos1, 900 - 1*160), (overlay_set_position, "$g_presentation_obj_custom_battle_designer_4", pos1),   
     (position_set_x, pos1, 900 - 2*160), (overlay_set_position, "$g_presentation_obj_custom_battle_designer_5", pos1),  
     (position_set_x, pos1, 900 - 3*160), (overlay_set_position, "$g_presentation_obj_custom_battle_designer_6", pos1),
	 (create_game_button_overlay, "$g_presentation_obj_custom_battle_designer_7", "@Mod Options"),
     (position_set_x, pos1, 900),(position_set_y, pos1, 75),(overlay_set_position, "$g_presentation_obj_custom_battle_designer_7", pos1),
	]),
   ...
   (ti_on_presentation_event_state_change, 
    [(store_trigger_param_1, ":object"),
     ...
     (else_try),
       ...
       (call_script, "script_update_key_config_buttons"),                   # Update Captions 
	 (else_try),
       (eq, ":object", "$g_presentation_obj_custom_battle_designer_6"),     # DISABLE ALL pressed?
       (try_for_range, ":no", slot_keys_begin, slot_keys_begin + number_of_keys),           
           (troop_set_slot, key_config_data, ":no", 0xff),                          # Disable all keys
       (try_end),
       (call_script, "script_update_key_config_buttons"),                           # Update Captions 
     (else_try),
       (eq, ":object", "$g_presentation_obj_custom_battle_designer_7"),     # RETURN TO PREFERENCES
	   (call_script, "script_set_global_var_key_config"),                   # Write the Config to Global Variable
