   ...
   [
     (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
	   ...
	   (try_begin),
		 ...
		 (call_script, "script_cf_spawn_side_cannons", ":party_no"),  
	   (try_end),
	   ##drtomas - cannons
     ...
     (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
       ...
       (party_slot_eq, ":center_no", slot_center_is_besieged_by, -1), #center not under siege
		...
		##drtomas begin
	   (store_party_size, ":size", ":center_no"),
	   (assign, ":continue", 0),
	   (try_begin), ##towns
...
#	(assign, ":controversy_deduction", 1),
   ...
   (8, #increased from 12
	...
	#Added Nov 2010 ends
	(call_script, "script_cf_random_political_event"),
	#CelestialRed TOTSK edit - 3rd time due to faction numbers and sizes.
 ...
 #     ]),
  ...
  # Process vassal ai
   ...
   [
	...
	##DrTomas 1257ad script
	##made - new trigger for alarms. they will react only to sieges/raids and will be more effective in response.
	(1,
	[
	  (try_for_range, ":center_no", centers_begin, centers_end),
  ...
  # Process alarms - perhaps break this down into several groups, with a modula
   ...
   [
     ##(call_script, "script_process_alarms"),##DrTomas - this sucks
...
##DRTOMAS - this needs to go, parties chop up too much CPU
...
# #         (display_message, "@Village farmers created at {s1}."),
       # (try_end),
    # ]),
##DrTomas new script to emulate the trading
  (8,
   [
       (try_for_range, ":village_no", villages_begin, villages_end),
         (party_slot_eq, ":village_no", slot_village_state, svs_normal),
 ...
 #Update how good a target player is for bandits
  ...
  #(1,
   [
     ...
     (call_script, "script_update_recruit_parties_2"), #TOTSK - this now have militia
    # (call_script, "script_update_recruit_parties"), #TOTSK #DrTomas - the second script sucks
     #NPC changes begin
  ...
  [
    (try_for_parties, ":center_no"),#Floris Population Resetting // Forced Recruiting
      (this_or_next|party_slot_eq, ":center_no", slot_center_recruits, 0),
	  (party_slot_eq, ":center_no", slot_center_recruits, 1),
      (party_set_slot, ":center_no", slot_center_recruits, 1),
	 (else_try),
  ...
  (12,
   [(try_for_range, ":center_no", centers_begin, centers_end),
      ...
      (try_end),
	  (try_begin),
        (eq, ":cur_improvement", slot_center_has_church),
		(party_get_slot,":lord", ":center_no", slot_town_lord),
		(call_script, "script_change_troop_renown", ":lord", 10),
      (try_end),
    (try_end),
    ]),
...
#    (ge, "$g_last_rest_center", 0),
  ...
  # Reduce renown slightly by 0.5% every week - Edited out by CelestialRed
  #(7 * 24,
  # [
  #    (troop_get_slot, ":player_renown", "trp_player", slot_troop_renown),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (1,
   [
     ...
     (store_add, ":fac_kingdom_15_plus_one", "fac_kingdom_15", 1),

     (try_for_range, ":faction_1", "fac_kingdom_1", ":fac_kingdom_15_plus_one"),
       (try_for_range, ":faction_2", "fac_kingdom_1", ":fac_kingdom_15_plus_one"),
...
####################################################################################################################################
  ##remove regiments if the HQ changed faction. Ie was conquered
  (24,
  [
	  (call_script, "script_hq_check"),
  ]),
   
  (24 * 3,
   [
	  (call_script, "script_reinforce_all_regiments"),
   ]),
