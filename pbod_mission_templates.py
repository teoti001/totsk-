...
pbod_battle_triggers = [
  #Fix for setting divisions, duplicated in formations code, so disabled in mst_lead_charge
  (ti_on_agent_spawn, 0, 0, [(store_cur_mission_template_no, reg0),(neq, reg0, "mt_lead_charge")], [(store_trigger_param_1, ":agent"),(call_script, "script_prebattle_agent_fix_division", ":agent")]),
  (0.5, 0, 0, [(store_cur_mission_template_no, reg0),(neq, reg0, "mt_lead_charge"),
...
real_deployment_triggers = [
 (0, 0.8, ti_once, [(mission_cam_set_screen_color, 0xFF000000),],
   [
	...
	(game_key_get_key, reg1, gk_action),
	(call_script, "script_str_store_key_name", s49, reg1),
	(game_key_get_key, reg1, gk_order_1),
	(call_script, "script_str_store_key_name", s48, reg1),
	...
	(party_set_slot, 0, slot_party_pref_real_deployment, 1), #reset to standard preference on
	(party_set_slot, 1, slot_party_pref_real_deployment, 0), #counter
	(party_set_slot, 2, slot_party_pref_real_deployment, reg0), #max orders 	
 ...
 (0, 1.3, ti_once, [(neg|is_vanilla_warband),(party_slot_eq, "p_main_party", slot_party_pref_real_deployment, 2)], #activated by "Take the Field" or "Enough Planning"
   [
	...
	(party_set_slot, 2, slot_party_temp_cheatmode, reg0), #so player's cheat mode setting is preserved
 ...
 (0, 0, 0, [(eq, "$battle_phase", BP_Spawn),(game_key_released, gk_order_1), #hold down-F1 flag released
 	(neg|is_presentation_active, "prsnt_battle"), #it surpresses the hold-F1 flag
	(store_mission_timer_c_msec, reg0),
	(val_sub, reg0, "$when_f1_first_detected"),
	(ge, reg0, 1), #since time is slowed down
	], 
   [
    #(stop_time, 1),
	...
	(try_for_range, ":division", 0, 9),
		...
		(try_begin),
			(this_or_next|eq, ":fformation", formation_none), #to force a line
	...
	(party_get_slot, ":order_count", 1, slot_party_pref_real_deployment), #counter
	(val_add, ":order_count", 1),
	(party_set_slot, 1, slot_party_pref_real_deployment, ":order_count"),
	(party_get_slot, ":max_orders", 2, slot_party_pref_real_deployment),
	(store_sub, reg0, ":max_orders", ":order_count"),
	...
	(eq, reg0, 0),
	(party_set_slot, 2, slot_party_pref_real_deployment, -1), #end		
   ]),
   
 #End  
 (0, 0.0005, ti_once, [(eq, "$battle_phase", BP_Spawn),(this_or_next|game_key_clicked, gk_action),(party_slot_eq, 2, slot_party_pref_real_deployment, -1),
	...
	(mission_cam_animate_to_position, cam_position, 1, 1),
	],  #delay of .00075 okish too
   [
    (assign, "$when_f1_first_detected", 0), #so the hold-F1 formations is overriden
	(party_set_slot, "p_main_party", slot_party_gk_order, 0),
	(party_set_slot, 2, slot_party_gk_order_hold_over_there, 0), #as holdit
	
	(party_set_slot, 1, slot_party_pref_real_deployment, 0), #counter
	(party_set_slot, 2, slot_party_pref_real_deployment, 0), #max orders 
	(assign, "$battle_phase", BP_Setup),
    #(stop_time, 0),
    (set_fixed_point_multiplier, 10),
	(mission_set_time_speed, 10),
	(party_get_slot, reg0, 2, slot_party_temp_cheatmode), #so player's cheat mode setting is preserved
...
prebattle_deployment_triggers  = [
 ...
 (ti_after_mission_start, 0, 0, [(party_slot_eq, "p_main_party", slot_party_prebattle_customized_deployment, 1)], [
	...
	(neq, ":target_num_of_stacks", ":starting_num_of_stacks"),
	#maintain prior stack order
	(party_get_num_companion_stacks, ":num_of_stacks", "p_main_party"),
...
caba_order_triggers = [
	...
	(0, 0, 0, [(game_key_clicked, gk_order_4),(neg|main_hero_fallen)], [
		(try_begin),
			...
			(store_cur_mission_template_no, reg0),(eq, reg0, "mt_lead_charge"),
...
def modmerge_mission_templates(orig_mission_templates):
	...
	for i in range(len(orig_mission_templates)):
		...
		if( mt_name=="lead_charge" or mt_name=="village_raid" or mt_name=="village_attack_bandits"):
			orig_mission_templates[i][5].extend(pbod_common_triggers+pbod_battle_triggers+prebattle_orders_triggers+prebattle_deployment_triggers+caba_order_triggers+field_ai_triggers+custom_camera_triggers)
		elif ("besiege" in mt_name or "castle_attack" in mt_name or mt_name=="entrenched_encounter" or mt_name=="ship_battle"):
			orig_mission_templates[i][5].extend(pbod_common_triggers+pbod_battle_triggers+prebattle_orders_triggers+prebattle_deployment_triggers+caba_order_triggers+custom_camera_triggers)
