...
store_trigger_param = 2074 #(store_trigger_param, <destination>, [<trigger_param_no>]), #Stores [<trigger_param_no>] into <destination>
...
agent_get_bone_position = 3314 #(agent_get_bone_position, <position_register>, <agent_no>, <bone_no>), #Stores <agent_no>'s <bone_no> position into <position_register>
agent_get_scale = 3315 #(agent_get_scale, <destination_fixed_point>, <agent_no>), #Stores <agent_no>'s scale into <destination_fixed_point>
agent_set_forced_lod = 3316 #(agent_set_forced_lod, <agent_no>, <lod_level>), #Forces <agent_no>'s LOD level to <lod_level> (0 = auto)
agent_get_item_slot_flags = 3317 #(agent_get_item_slot_flags, <destination>, <agent_no>, <item_slot_no>), #Stores <agent_no>'s <item_slot_no> item slot flags into <destination>
...
set_default_item_prune_time = 3601 #(set_default_item_prune_time, <type>, <prune_time>), #Sets the default prune time to <prune_time> (in seconds) for all dropped items of <type> spawned after the call. A negative value means item are persistent. Valid types are: 7 (missiles), 8 (broken shields), 9 (items dropped on death)
prop_instance_clear_attached_missiles = 3602 #(prop_instance_clear_attached_missiles, <prop_instance_no>), #Removes all attached missiles from <prop_instance_no>. Works only with dynamic scene props (non-retrievable missiles)
spawn_missile = 3603 #(spawn_missile, <agent_no>, <position>, <velocity_fixed_point>, <weapon_item_no>, <missile_item_no>, [<weapon_item_modifier>], [<missile_item_modifier>]), #Spawns a <missile_item_no> shot by <agent_no> with <weapon_item_no> at <position> with <velocity_fixed_point>
camera_in_first_person = 3604 #(camera_in_first_person), #Fails if the camera is not in first person
set_camera_in_first_person = 3605 #(set_camera_in_first_person, <value>), #Sets the camera to first or third person
set_show_use_tooltip = 3606 #(set_show_use_tooltip, <tooltip_type>, [<value>]), #Enables or disables use tooltips. See header_constants_addon.py for possible types
prop_instance_add_particle_system = 3607 #(prop_instance_add_particle_system, <prop_instance_no>, <particle_system_no>), #Adds <particle_system_no> to <prop_instance_no> using the position delta.
prop_instance_remove_particle_system = 3608 #(prop_instance_remove_particle_system, <prop_instance_no>, [<particle_system_no>]), #Removes [<particle_system_no>] from <prop_instance_no>. If [<particle_system_no>] is not specified or -1, all particle systems will be removed.
set_ally_collision_threshold = 3609 #(set_ally_collision_threshold, <low_boundary>, <high_boundary>), #Changes the animation progress boundaries (in percents) that determine if attacks on allies will collide (default: 45% <= x <= 60%)
set_prop_collision_threshold = 3610 #(set_prop_collision_threshold, <attack_direction>, <low_boundary>, <high_boundary>), #Changes the animation progress boundaries (in percents) that determine if swing attacks on props will collide (default: 40% <= x <= 80% (75% for overheads))
particle_system_remove = 3611 #(particle_system_remove, [<particle_system_no>]), #Removes [<particle_system_no>] (all particle systems if not set or -1) from the current entity (can be used in several in triggers)
get_camera_position = 3612 #(get_camera_position, <position_register_no>), #Stores camera position and rotation into <position_register_no>
get_spectated_agent_no = 3613 #(get_spectated_agent_no, <destination>), #Stores spectated agent no into <destination>
prop_instance_set_forced_lod = 3614 #(prop_instance_set_forced_lod, <prop_instance_no>, <lod_level>), #Forces <prop_instance_no>'s LOD level to <lod_level> (0 = auto)
prop_instance_set_variation_id = 3615 #(prop_instance_set_variation_id, <prop_instance_no>, <value>), #Sets <prop_instance_no>'s variation id to <value>
prop_instance_set_variation_id_2 = 3616 #(prop_instance_set_variation_id_2, <prop_instance_no>, <value>), #Sets <prop_instance_no>'s variation id 2 to <value>
stop_time = 3617 #(stop_time, <value>), #Stops/resumes the mission. Works only in singleplayer with cheat mode enabled.
cur_missile_get_path_point_position = 3618 #(cur_missile_get_path_point_position, <position_register>, <path_point_no>), #Stores the position of the missile's <path_point_no> (0-499) into <position_register> (can be used in ti_on_init_missile)
