...
cannons_count = ( ##this might need changing - problematic situation can arrise
  ...
  [
    (troop_set_slot, "trp_cannon_array", 0, 0),
  ]
)

cannons_spawn = (
  ti_after_mission_start, 3, 0, [],
  [
    (assign, "$total_cannon_count", 0), 
    (assign, ":amount", 0),
	##enemy count!
	(try_for_parties, ":party"),
	  (neq, ":party", "p_main_party"),
	  (lt, ":amount", cannon_count_per_side),
	  (party_is_active, ":party"),
	  (party_get_battle_opponent, ":opponent",":party"),
	  (this_or_next|eq, ":opponent", "p_main_party"), #fight the player
	  (eq, ":opponent","$g_encountered_party_2"), #or a third party
	  ##(display_message, "@found enemy!"),
	  (call_script, "script_get_cannon_count", ":party"),
	  (gt, reg1, 0), ##has cannons
	  (try_for_range, reg1, 0, reg4),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", enemy_team, "spr_twleve_pound_cannon", ":party", 1),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 3),
		(val_add, ":amount", 1),
	  (try_end),
	  (try_for_range, reg1, 0, reg3),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", enemy_team, "spr_six_pound_cannon", ":party", 1),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 2),
		(val_add, ":amount", 1),
	  (try_end),
	  (try_for_range, reg1, 0, reg2),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", enemy_team, "spr_three_pound_cannon", ":party", 1),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 1),
		(val_add, ":amount", 1),
	  (try_end),
	(try_end),
	##player cannons
	...
	(try_begin),
	  ...
	  (try_for_range, reg1, 0, reg4),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", player_team, "spr_twleve_pound_cannon", "p_main_party", 3),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 3),
		(val_add, ":amount", 1),
	  (try_end),
	  (try_for_range, reg1, 0, reg3),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", player_team, "spr_six_pound_cannon", "p_main_party", 3),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 2),
		(val_add, ":amount", 1),
	  (try_end),
	  (try_for_range, reg1, 0, reg2),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", player_team, "spr_three_pound_cannon", "p_main_party", 3),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 1),
		(val_add, ":amount", 1),
	  (try_end),
	(try_end),
	##ally cannons
	(try_for_parties, ":party"),##ally count!
	  (neq, ":party", "p_main_party"),
	  (lt, ":amount", cannon_count_per_side),
	  (party_is_active, ":party"),
	  (party_get_battle_opponent, ":opponent",":party"),
	  #(this_or_next|eq, ":opponent", "p_main_party"), #fight the player
	  (eq, ":opponent","$g_encountered_party"), #third party
	  ##(display_message, "@found ally!"),
	  (call_script, "script_get_cannon_count", ":party"),
	  (gt, reg1, 0), ##has cannons
	  (try_for_range, reg1, 0, reg4),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", ally_team, "spr_twleve_pound_cannon", ":party", 3),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 3),
		(val_add, ":amount", 1),
	  (try_end),
	  (try_for_range, reg1, 0, reg3),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", ally_team, "spr_six_pound_cannon", ":party", 3),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 2),
		(val_add, ":amount", 1),
	  (try_end),
	  (try_for_range, reg1, 0, reg2),
		(lt, ":amount", cannon_count_per_side),
		(call_script, "script_spawn_cannon", ":amount", ally_team, "spr_three_pound_cannon", ":party", 3),
		(scene_prop_set_slot, reg0, slot_spr_damage_multiplayer, 1),
		(val_add, ":amount", 1),
	  (try_end),
	(try_end),
	(troop_set_slot, "trp_cannon_array", 0, "$total_cannon_count"),
  ]
)

cannons_operation_check = (
  1, 0, 0, [],
  [
	(troop_get_slot, ":amount", "trp_cannon_array",  0), #get cannon count
	(val_add, ":amount", 1), #adjust for loop
	(try_for_range, ":slot", 1, ":amount"),
	  (troop_get_slot, ":cannon", "trp_cannon_array", ":slot"),	  
	  (scene_prop_set_slot,":cannon", slot_spr_cannon_op, op_non_funcional),
	  (scene_prop_get_slot,":captain", ":cannon", slot_spr_captain),
	  (scene_prop_get_slot,":sidekick", ":cannon", slot_spr_sidekick),
	  (scene_prop_get_slot,":sidekick2", ":cannon", slot_spr_sidekick2),
	  (try_begin), #everyone is alive
	    (agent_is_alive, ":captain"),
	    (this_or_next|agent_is_alive, ":sidekick"),
	    (agent_is_alive, ":sidekick2"),
        (agent_is_active, ":captain"),
	    (this_or_next|agent_is_active, ":sidekick"),
	    (agent_is_active, ":sidekick2"),
		(scene_prop_set_slot,":cannon", slot_spr_cannon_op, op_funcional),
	  (else_try),#captain is dead
	    (neg|agent_is_alive, ":captain"),
		##retreat!
		(agent_set_slot, ":sidekick", slot_agent_courage_score, 1),
		(agent_set_slot, ":sidekick2", slot_agent_courage_score, 1),
		(agent_clear_scripted_mode, ":sidekick"),
		(agent_clear_scripted_mode, ":sidekick2"),
	  (else_try), #both sidekicks is dead
		(neg|agent_is_alive, ":sidekick"),
		(neg|agent_is_alive, ":sidekick2"),
		##retreat!
		(agent_set_slot, ":captain", slot_agent_courage_score, 1),
		(agent_clear_scripted_mode, ":captain"),
	  (try_end),
...
cannons_select_targets = (
  ...
  [
	...
	(try_for_range, ":slot", 1, ":amount"), 
	  (troop_get_slot, ":cannon", "trp_cannon_array", ":slot"),	  
	  (scene_prop_slot_eq, ":cannon", slot_spr_cannon_op, op_funcional),
	  (scene_prop_get_slot,":cur_target", ":cannon", slot_spr_target_agent),
	  (scene_prop_get_team, ":cannon_team", ":cannon"),
	  ##check if target is alive -if it is, then no need for new one
	  (assign, ":continue", 1),
	  (try_begin),
	    (gt, ":cur_target", 0),
		(agent_is_alive, ":cur_target"),
		(agent_is_active, ":cur_target"),
		(assign, ":continue", 0),
	  (try_end),
	  (eq, ":continue", 1),
	  ##set up allies
	  (try_begin),
	    (this_or_next|eq, ":cannon_team", enemy_team),
	    (eq, ":cannon_team", enemy_team2),
		(assign, ":team1", enemy_team),
	    (assign, ":team2", enemy_team2),
	  (else_try),
	    (assign, ":team1", player_team),
	    (assign, ":team2", ally_team),
	  (try_end),
	  ...
	  (try_for_agents, ":agent"),
		...
		(neq, ":team", ":team1"), #not ally nor its team
		(neq, ":team", ":team2"),
		(try_begin),
		  (eq, ":cur_target", 0),
		  (assign, ":cur_target", ":agent"),
		(try_end),
		#lets randomize the selection a bit
		(store_random_in_range, ":random", 0, 99), 
		(le, ":random", 5),
		(assign, ":cur_target", ":agent"),
	  (try_end),
	  (scene_prop_set_slot, ":cannon", slot_spr_target_agent, ":cur_target"),
	(try_end),
  ]
)
cannons_rotate_to_target = (
  3, 0, 0, [],
  [
	...
	(try_for_range, ":slot", 1, ":amount"),
	  ...
	  (troop_get_slot, ":cannon", "trp_cannon_array", ":slot"),
	  (scene_prop_slot_eq, ":cannon", slot_spr_cannon_op, op_funcional),
	  (scene_prop_slot_eq, ":cannon", slot_spr_cannon_reloding, cannon_ready),
	  (scene_prop_get_slot,":agent", ":cannon", slot_spr_target_agent),
	  (gt, ":agent", 0),
	  
	 
      ## (agent_get_position, pos1, ":agent"),
	  ## (prop_instance_get_position, pos2, ":cannon"),
	  (call_script, "script_brute_rotation_check", ":agent", ":cannon"), ## this is a bit dumb, but oh well
	  (assign, ":direction", reg0),
	  (assign, ":rotation", reg1),
	  
	  
	   ##get and set up initial positions
	  (prop_instance_get_position, pos2, ":cannon"),
	  (val_mul, ":rotation", ":direction"),
	  
	  (try_begin),
		(eq, ":rotation", 0),
		(scene_prop_set_slot, ":cannon", slot_spr_cannon_rotation, cannon_static),
		##(display_message, "@static"),
	  (else_try),
	    (scene_prop_set_slot, ":cannon", slot_spr_cannon_rotation, cannon_rotating),
	  ...
	  (prop_instance_stop_animating, ":cannon"),
	  (prop_instance_animate_to_position, ":cannon", pos2, 300),
	  
	  ##get captain and sidekciks
	  (scene_prop_get_slot,":captain", ":cannon", slot_spr_captain),
	  (scene_prop_get_slot,":sidekick", ":cannon", slot_spr_sidekick),
	  (scene_prop_get_slot,":sidekick2", ":cannon", slot_spr_sidekick2),
	  (scene_prop_get_slot,":operator", ":cannon", slot_spr_agent_for_reload),
	  ##set captain behind cannon
	  (position_move_y, pos2, cannon_captain_adjust_y, 0), #back
	  (agent_set_scripted_destination, ":captain", pos2, 1),
	  (position_move_y, pos2, -cannon_captain_adjust_y, 0), #back
	  (prop_instance_get_position, pos2, ":cannon"),
	  ##set sidekick on the right of cannon
	  (assign, ":in_position", 0),
	  (try_begin),
	    (neq, ":operator", ":sidekick"),
		(agent_is_alive, ":sidekick"),
		(agent_is_active, ":sidekick"),
	    (position_move_x, pos2, cannon_sidekicks_adjust_x, 0), #back
		(position_set_z_to_ground_level, pos2),
	    (agent_set_scripted_destination, ":sidekick", pos2, 0),
		(agent_get_position, pos4, ":sidekick"),
		
		(position_set_z_to_ground_level, pos4),
		(get_distance_between_positions, ":distance", pos4, pos2),
		(assign, reg0, ":distance"),
		##(display_message, "@distance {reg0}"),
		(le, ":distance", 150), #meter
		(val_add, ":in_position", 1),
	  (try_end),
	  ##set sidekick2 on the left of cannon
	  (try_begin),
	    (neq, ":operator", ":sidekick2"),
		(agent_is_alive, ":sidekick2"),
		(agent_is_active, ":sidekick2"),
	    (position_move_x, pos2, -cannon_sidekicks_adjust_x * 2, 0), #back
		(position_set_z_to_ground_level, pos2),
	    (agent_set_scripted_destination, ":sidekick2", pos2, 0),
		(agent_get_position, pos4, ":sidekick2"),
		...
		(val_add, ":in_position", 1),
	  (try_end),
	  ##it's rotating if not all of the captain/sidekick/sidekick2
	  (try_begin),
	    (assign, ":quote", 0),
		(try_begin),
		  (agent_is_alive, ":sidekick"),
		...
		(try_begin),
		  ...
		  (val_add, ":quote", 1),
		(try_end),
		(neq, ":in_position", ":quote"), ##if alive agents are less then those who need to operate it
		(scene_prop_set_slot, ":cannon", slot_spr_cannon_rotation, cannon_rotating), #then rotation is set
	    ##(display_message, "@operators not in position"),
	  (try_end),
	(try_end),
  ]
)
...
cannons_reload = (
  ...
  [
	...
	(try_for_range, ":slot", 1, ":amount"),
	   (troop_get_slot, ":cannon", "trp_cannon_array", ":slot"),
	   (scene_prop_slot_eq, ":cannon", slot_spr_cannon_op, op_funcional),
	   (scene_prop_slot_eq, ":cannon", slot_spr_cannon_reloding, cannon_needs_loading), #not reloaded
	   (scene_prop_get_slot,":seconds", ":cannon", slot_spr_loading_seconds),
	   (scene_prop_get_slot,":captain", ":cannon", slot_spr_captain),
	   (scene_prop_get_slot,":sidekick", ":cannon", slot_spr_sidekick),
	   (scene_prop_get_slot,":sidekick2", ":cannon", slot_spr_sidekick2),
	   (scene_prop_get_slot,":operator", ":cannon", slot_spr_agent_for_reload),
	   
	   (scene_prop_set_slot, ":cannon", slot_spr_agent_for_reload, 0),
	   (try_begin),
	     (this_or_next|le, ":operator", 0),
		 (neg|agent_is_alive, ":operator"),
		 (try_begin),
		   (agent_is_alive, ":sidekick"),
		   (agent_is_active, ":sidekick"),
		   (assign, ":operator", ":sidekick"),
		 (else_try),
		   (agent_is_alive, ":sidekick2"),
		   (agent_is_active, ":sidekick2"),
		   (assign, ":operator", ":sidekick2"),
		 (try_end),
	   (try_end),
	   (gt, ":operator", 0),
	   ##(display_message, "@cannon has operator"),
	   (agent_get_position, pos2, ":operator"),
	   (prop_instance_get_position, pos1, ":cannon"),
	   (position_move_z, pos1, 120, 0),
	   (copy_position, pos3, pos1),
	   (position_move_y, pos3, 300, 0),
	   (position_rotate_z, pos3, 180, 0),
	   
	   (agent_set_scripted_destination, ":operator", pos3, 1),
	   (get_distance_between_positions_in_meters, ":distance", pos3, pos2),
	   (lt, ":distance", 1),
	   ##(display_message, "@cannon operator in place"),
	   
	   (try_begin),
	     (gt, ":seconds", 0),
		 (val_sub, ":seconds", 1),
		 #(agent_set_look_target_position,":operator", pos1),
		 (try_begin), 
			##this to prevent possible crashes?
		   (agent_is_alive, ":captain"),
		   (agent_is_active, ":captain"),
		   (agent_is_alive, ":operator"),
		   (agent_is_active, ":operator"),
		   (agent_set_look_target_agent, ":operator", ":captain"),
		   (agent_set_attack_action, ":operator", 0, 0),
		   #(agent_set_animation, ":operator", "anim_release_thrust_staff", 1), 
		 (try_end),
		 (scene_prop_set_slot, ":cannon", slot_spr_loading_seconds, ":seconds"),
		 ##(assign, reg0, ":seconds"),
		 ##(display_message, "@reloading timer: {reg0}"),
		 (scene_prop_set_slot, ":cannon", slot_spr_agent_for_reload, ":operator"),
		 (scene_prop_set_slot, ":cannon", slot_spr_cannon_rotation, cannon_rotating), #extra protection for it not to shoot if there are an agent reloading
	   (else_try),
	     (scene_prop_set_slot, ":cannon", slot_spr_cannon_reloding, cannon_ready),
	     (scene_prop_set_slot, ":cannon", slot_spr_agent_for_reload, 0),
		 ##(display_message, "@cannon reloaded"),
	   (try_end),
	(try_end),
  ]
)
...
cannons_shoot = (
  ...
  [
	...
	(try_for_range, ":slot", 1, ":amount"),
	   ...
	   (scene_prop_slot_eq, ":cannon", slot_spr_cannon_op, op_funcional),
	   ##(display_message, "@cannon functional"),
	   (scene_prop_slot_eq, ":cannon", slot_spr_cannon_reloding, cannon_ready),
	   ##(display_message, "@cannon loaded"),
	   (scene_prop_slot_eq, ":cannon", slot_spr_cannon_rotation, cannon_static),
	   ##(display_message, "@cannon targeted"),
	   (scene_prop_slot_ge, ":cannon", slot_spr_target_agent, 0),
	   ##(display_message, "@cannon has agent"),
	   (scene_prop_get_team, ":cteam", ":cannon"),
	   (team_get_hold_fire_order, ":fire_order", ":cteam", cannon_division),
	   (neq, ":fire_order", 1),
	   ...
	   (position_move_z, pos1, 120, 0),
	   (copy_position,pos3,pos1),
	   (position_move_z, pos3, -1000, 0),
	   (set_spawn_position, pos3),
	   (spawn_scene_prop, "spr_cannon_ball"),
	   (assign, ":ball", reg0),	   
	   
	   (position_get_x, ":x", pos2),
	   (position_get_y, ":y", pos2),
	   (store_random_in_range, ":rx", -crap_factor, crap_factor),
	   (store_random_in_range, ":ry", -crap_factor, crap_factor),
	   (val_add, ":x", ":rx"),
	   (val_add, ":y", ":ry"),
	   
	   (scene_prop_set_slot, ":ball", slot_balls_target_x, ":x"),
	   (scene_prop_set_slot, ":ball", slot_balls_target_y, ":y"),
	   (get_distance_between_positions_in_meters,":distance",pos1,pos2),
	   (store_div, ":speed", ":distance", 100),
	   (scene_prop_set_slot, ":ball", slot_balls_target_speed, ":speed"),
	   (scene_prop_set_slot, ":ball", slot_balls_target_moving, ball_falling),
	   (scene_prop_set_slot, ":ball", slot_balls_cannon, ":cannon"),
	   
	   (scene_prop_set_slot, ":cannon", slot_spr_cannon_reloding, cannon_needs_loading),
	   
	   
	   ##(display_message, "@cannon fired"),
	   ##(display_message, "@cannon ammo depleted"),
	   #(particle_system_burst,"psys_pan_smoke",pos1,100),
	   (particle_system_burst,"psys_dummy_smoke",pos1,100),
	   (particle_system_burst,"psys_firearm_flash",pos1,100),
	   (prop_instance_play_sound, ":cannon", "snd_cannon_shoot"),
	   ## (display_message, "@smoke and fire"),
	   (store_random_in_range, ":seconds", 0, 10),
	   (val_add, ":seconds", cannon_reload_timer),
	   (scene_prop_set_slot, ":cannon", slot_spr_loading_seconds, ":seconds"),
	(try_end),
  ]
)
...
cannons_balls_move = (
  ...
  [
	(set_fixed_point_multiplier, 100),
	(scene_prop_get_num_instances, ":count", "spr_cannon_ball"),
	(try_for_range, reg0, 0, ":count"),
	  (scene_prop_get_instance, ":ball", "spr_cannon_ball", reg0),
	  (neg|scene_prop_slot_eq, ":ball", slot_balls_target_moving, ball_landed),
	  (scene_prop_get_slot, ":speed", ":ball", slot_balls_target_speed),
	  (scene_prop_get_slot, ":cannon", ":ball", slot_balls_cannon),
	  (scene_prop_get_slot, ":captain", ":cannon", slot_spr_captain),
	  (scene_prop_get_slot, ":dmg", ":cannon", slot_spr_damage_multiplayer),
	  (try_begin),
	    (gt, ":speed", 0),
		(val_sub, ":speed", 1),
	    (scene_prop_set_slot, ":ball", slot_balls_target_speed, ":speed"),
	  (else_try),
		(scene_prop_get_slot, ":x", ":ball", slot_balls_target_x),
		(scene_prop_get_slot, ":y", ":ball", slot_balls_target_y),
		(init_position, pos2),
		(position_set_x, pos2, ":x"),
		(position_set_y, pos2, ":y"),
		(position_set_z_to_ground_level, pos2),
		(prop_instance_set_position, ":ball", pos2),
		(set_spawn_position, pos2),
		##(spawn_scene_prop, "spr_battle_field_smoke"), ##too silly
		(particle_system_burst,"psys_firearm_flash",pos2,10),
		(particle_system_burst,"psys_village_fire_big",pos2,25),
		(particle_system_burst, "psys_dummy_smoke_big",pos2,100),
		(prop_instance_play_sound, ":ball", "snd_cannon_explosion"),
		
		(scene_prop_set_slot, ":ball", slot_balls_target_moving, ball_landed),
		##(display_message, "@ball hit the ground"),
		##deliver damage
		(try_for_agents, ":agent"),
		  (agent_is_alive, ":agent"),
		  (agent_is_active, ":agent"),
		  (agent_get_position, pos1, ":agent"),
		  (get_distance_between_positions_in_meters, ":distance",pos1,pos2),
		  (store_random_in_range, ":damage", 50, 101),
		  (try_begin),
		    (le, ":distance", 2),
			(val_mul, ":damage", ":dmg"),
			(agent_deliver_damage_to_agent, ":captain", ":agent", ":damage"), #200
		  (else_try),
		    (le, ":distance", 4),
			(agent_deliver_damage_to_agent, ":captain", ":agent", ":damage"), #100
		  (try_end),
		(try_end),
	  (try_end),
	(try_end),
...
remove_buildings = (
  ...
  [
	(call_script, "script_remove_dynamic_buildings"),
  ]
)
...
deathcam_mouse_deadzone = 1 #set this to a positive number (MV: 2 or 3 works well for me, but needs testing on other people's PCs)
...
initialise_auxiliary_player = (ti_before_mission_start, 0, 0, 
	...
	[
		...
		(assign, "$g_dplmc_battle_continuation", 1), #enable deathcam
		...
		(assign, "$g_dplmc_battle_continuation", 0), #disable deathcam, until we can not find any troops to "posses"
	])

auxiliary_player_check = (5, 0, 0, 
	  [
	    (eq, "$use_player_auxiliary", 1),
	    (eq, "$g_dplmc_battle_continuation", 0), #if deatcham is activated, no longer spawn as an auxiliary. TODO: reset camera settings and spawn anyway?
	  ...
	  [
		...
		(try_end),  
		(eq, ":spawned", 0),
		(assign, "$g_dplmc_battle_continuation", 1), #deathcam is active now
	  ])
    ...
    [(0,mtef_scene_source|mtef_team_0,af_override_horse,0,1,pilgrim_disguise),
     ...
     [
      ...
      [
		...
		(is_between, ":wielded_item", "itm_stones", "itm_ottoman_musket_b"),
...
#-----------------------------------------------------------------
  ...
  (
   ...
   [
     ...
     (try_begin), 
       ...
       (this_or_next|eq, ":dead_agent_troop_no", "trp_british_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_austrian_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_bavaria_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_dutch_prison_guard"),
	   (this_or_next|eq, ":dead_agent_troop_no", "trp_bourbon_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_danish_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_swedish_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_prussia_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_habsburg_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_polish_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_russia_prison_guard"),
       (eq, ":dead_agent_troop_no", "trp_ottoman_prison_guard"),
  ...
  (
    ...
    "village_training",
    [(2,mtef_visitor_source|mtef_team_0,af_override_everything,aif_start_alarmed,1,[itm_practice_sword, itm_linen_tunic, itm_khergit_leather_boots]),
     (4,mtef_visitor_source|mtef_team_1,af_override_everything,aif_start_alarmed,1,[itm_practice_sword, itm_linen_tunic, itm_khergit_leather_boots]),
     ],
    [
      (ti_before_mission_start, 0, 0, [],
       [
  ...
  (
    ...
    [(0,mtef_team_0|mtef_use_exact_number,af_override_horse|af_override_weapons|af_override_head,aif_start_alarmed,4,[itm_rapier_5]),
     (3,mtef_visitor_source|mtef_team_1,af_override_horse,aif_start_alarmed,1,[]),
     ],
    [
      common_inventory_not_available,
    ...
    [
      (ti_on_agent_spawn, 0, 0, [],
       [
		 ...
		 #DrTomas - cannon troops morale
		 (try_begin),
		   (ge, ":agent_party", 0),
		   (party_get_morale, ":cur_morale", ":agent_party"),
		 (else_try),
           (assign, ":cur_morale", 50), #middle value?
         (try_end),
		 #DrTomas - cannon end
...
##          (agent_get_team, reg3, ":dead_agent_no"),          
  ...
  (
    ...
    ] + dynamic_villages + auxiliary_player,
  ),
...
#      (ti_before_mission_start, 0, 0, [], [(set_rain, 1,100)]),
  ...
  (
    ...
    [
      (0,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic,itm_khergit_leather_boots]),
      (1,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_linen_tunic,itm_khergit_leather_boots]),
      (2,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic,itm_khergit_leather_boots]),
      (3,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic,itm_khergit_leather_boots]),
      (4,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (5,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic,itm_khergit_leather_boots]),
      (6,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic,itm_khergit_leather_boots]),
      (7,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, ]),

      (8,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (9,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (10,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (11,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (12,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (13,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (14,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (15,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),

      (16,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (17,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (18,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (19,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (20,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (21,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (22,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (23,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),

      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (25,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (26,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (27,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (28,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (29,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (30,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (31,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
#32
      (32, mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_heavy_practice_sword]),
      (33,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_practice_dagger]),
      (34,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword]),
      (35,mtef_visitor_source|mtef_team_4,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_practice_dagger]),
      (36, mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_practice_dagger]),
      (37,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_practice_dagger]),
      (38,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_practice_dagger]),
      (39,mtef_visitor_source|mtef_team_4,af_override_all,aif_start_alarmed,1,[itm_practice_sword, itm_practice_dagger]),
#40-49 not used yet
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (24,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
...
#used for torunament master scene
      ...
      (56, mtef_visitor_source|mtef_team_0, af_override_all, aif_start_alarmed, 1, [itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
      (57, mtef_visitor_source|mtef_team_0, af_override_all, aif_start_alarmed, 1, [itm_practice_sword,itm_dueling_shirt, itm_practice_dagger,itm_gentlemens_shoes]),
    ],
    tournament_triggers
  ),
  ...
  (
    ...
    "You enter a melee fight in the arena.",
    [    
	  (0, mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_linen_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
	  (16, mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_villager_blue_tunic, itm_practice_dagger,itm_khergit_leather_boots]),
    ],
    [
      common_inventory_not_available,
      (ti_tab_pressed, 0, 0, [(display_message, "str_cannot_leave_now")], []),
...
##  ),
  ...
  (
    ...
    "Wedding",
    [
        (0,mtef_visitor_source,af_override_everything,0,1,[itm_gentleman_outfit_a, itm_white_stocking_buckle_shoes,itm_officer_smallsword,itm_white_gloves]),#Groom
        (1,mtef_visitor_source,af_override_everything,0,1,[itm_bride_dress, itm_bride_crown, itm_bride_shoes,itm_white_gloves]),#Bride
        (2,mtef_visitor_source,af_castle_lord,0,1,[]),
        (3,mtef_visitor_source,af_override_everything,0,1,[itm_gentleman_outfit_d, itm_white_stocking_buckle_shoes,itm_officer_smallsword,itm_white_gloves,itm_gentleman_wig_a]),#Priest
...
##         (try_end),
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only,af_override_everything,0,1,[itm_tutorial_shield,itm_tutorial_sword,itm_tutorial_short_bow,itm_tutorial_arrows,itm_dueling_shirt,itm_gentlemens_shoes]), #af_override_weapons
     ],
    [
      (ti_tab_pressed, 0, 0, [],
       [(try_begin),
...
##  ),
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only,af_override_everything,0,1,[itm_tutorial_shield,itm_tutorial_sword,itm_tutorial_short_bow,itm_tutorial_arrows,itm_dueling_shirt,itm_gentlemens_shoes]), #af_override_weapons #af_override_weapons
     ],
    [
      (ti_tab_pressed, 0, 0, [],
       [(try_begin),
    ...
    [
      ...
      (0, 0, 0, [(try_begin),
                 ...
                 (else_try),
                   ...
                   (reset_visitors),
                   (set_visitor,5,"trp_watchman"),
                   (set_visitor,6,"trp_caravan_guard"),
                   (set_visitor,7,"trp_mercenary_swordsman"),
